/* Template angles-9x16 — AI Edit vs Original 9:16 */
(function () {
  function Component(props) {
    ...
    const chipStyle = { backgroundImage: "linear-gradient(90deg, rgba(0, 0, 0, 0.6) 0%, rgba(0, 0, 0, 0.6) 100%), linear-gradient(90deg, rgba(120, 120, 130, 0.45) 0%, rgba(120, 120, 130, 0.45) 100%)" };
    const Label = ({ swap, text }) => {
      const v = labelText[swap];
      return (
        <div data-pin data-swap={swap} className={"absolute top-[18px] left-[18px] z-10 flex items-center justify-center px-[18px] py-[11px] rounded-[14px]" + (window.__EDITOR ? "" : " pointer-events-none")} style={chipStyle}>
          {v === "@logo"
            ? <img src={window.__AO_LOGO} alt="Agent Opus" className="invert h-[28px] object-contain" />
            : <p className="font-medium leading-none text-[26px] text-[#e8e8ee] tracking-[0.2px] whitespace-nowrap font-[Geist,sans-serif]">{(v && v !== "@logo") ? v : text}</p>}
        </div>
      );
    };
    return (
      <div className="relative overflow-hidden bg-[#0c0e13]" style={{ width: 1080, height: 1920 }}>
        <div className="hidden" />
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center w-[960px]">
          <p className="font-semibold leading-none text-[52px] text-[#cfe0ff] text-center tracking-[0.6px] uppercase font-[Geist,sans-serif] whitespace-nowrap" style={{ textShadow: "0 0 24px rgba(140,180,255,.55)" }}>One camera to multiple angles?</p>
          <div className="relative mt-[64px] w-full h-[540px]">
            <Slot k="slot1" className="absolute inset-0 rounded-[24px] bg-[#9f9f9f]" />
            {Label({ swap: "label1", text: "AI Edit" })}
          </div>
          <div className="relative mt-[64px] w-full h-[540px]">
            <Slot k="slot2" className="absolute inset-0 rounded-[24px] bg-[#8f8f8f]" />
            {Label({ swap: "label2", text: "Original Footage" })}
          </div>
        </div>
      </div>
    );
  ...
  window.TEMPLATES.push({
    id: "angles-9x16",
    name: "AI Edit vs Original 9:16",
    width: 1080,
    height: 1920,
    slots: 2,
    desc: "Vertical comparison on near-black: a glowing question headline over two rounded clips labelled AI Edit and Original Footage.",
    Component
  });
})();
