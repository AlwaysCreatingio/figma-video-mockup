...
(function () {
  function Component(props) {
    ...
    React.useEffect(() => { if (window.__slotAPI) { window.__slotAPI.setLogos = setLogos; window.__slotAPI.setLogoScales = setLogoScales; window.__slotAPI.setLogoBgs = setLogoBgs; window.__slotAPI.setShowPlus = setShowPlus; } }, []);
        ...
        <div className="-translate-x-1/2 -translate-y-1/2 absolute flex flex-col gap-[42px] items-center left-1/2 top-1/2 w-[522.941px]">
          ...
          <div className="relative group flex flex-col items-center">
            <div className={"flex items-center relative shrink-0 " + (showPlus ? "gap-[18px]" : "gap-[33px]")}>
              <LogoSlot k="logo1" className="bg-[#565656] relative rounded-[14px] shrink-0 size-[80px]" />
              {showPlus && <span data-plus className="text-neutral-500 text-[30px] leading-none font-light">+</span>}
              <LogoSlot k="logo2" className="bg-[#565656] relative rounded-[14px] shrink-0 size-[80px]" />
              {showPlus && <span data-plus className="text-neutral-500 text-[30px] leading-none font-light">+</span>}
              <LogoSlot k="logo3" className="bg-[#565656] relative rounded-[14px] shrink-0 size-[80px]" />
            </div>
            <button data-ctl onClick={() => setShowPlus(v => !v)} className="absolute -top-8 right-0 opacity-0 group-hover:opacity-100 transition text-[11px] font-medium px-2.5 py-1 rounded-lg bg-neutral-800 text-white hover:bg-neutral-700">{showPlus ? "Remove +" : "Add +"}</button>
          </div>
